...
namespace zSprite
{
    ...
    public class FiberScheduler
    {
        ...
        public void tick()
        {
            ...
            while (!delayedByTime.IsEmpty && delayedByTime.Peek().Key < Root.instance.time.time)
                running.AddLast(delayedByTime.DequeueValue());

            //get all items that are ready to be dequeued by frames
            while (!delayedByFrames.IsEmpty && delayedByFrames.Peek().Key < Root.instance.time.frameCount)
        ...
        {
            delayedByTime.Enqueue((float)(ts.TotalSeconds + Root.instance.time.time), fiber);
        }

        public void schedule(IFiber fiber, float delay)
        {
            delayedByTime.Enqueue((float)(delay + Root.instance.time.time), fiber);
        }
        ...
        {
            delayedByFrames.Enqueue((int)(frameDelay + Root.instance.time.frameCount), fiber);
        }
